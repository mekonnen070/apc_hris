...
abstract class EmployeeInfoModel with _$EmployeeInfoModel {
  ...
  const factory EmployeeInfoModel({
    ...
    // For a "create new employee" DTO, these might be empty or handled in separate API calls.
    // For displaying a full employee profile, they would be populated.
    ...
    // Replace EmployeeUploadModel with a more specific name if used for a particular document category here
    // or use a generic list if EmployeeUpload is for various ad-hoc documents.
    // For now, assuming general document uploads associated with the employee:
    @Default([]) List<EmployeeUploadModel> generalDocuments,
    // Other collections from C# (EmploymentHistory, EmployeeLeaves, RetirementDetails, TrainingApplicant)
    // would follow the same pattern if needed:
    // @Default([]) List<EmploymentHistoryModel> employmentHistory,
    // @Default([]) List<EmployeeLeaveModel> employeeLeaves,
    // @Default([]) List<RetirementInfoModel> retirementDetails,
    // @Default([]) List<TrainingApplicantModel> trainingApplied,
...
// Extension for computed properties like 'fullName' from C#
extension EmployeeInfoModelX on EmployeeInfoModel {
  ...
  // The CalculateEligibility method from C# is business logic
  // and would typically reside in a notifier or service class in Dart,
  // not directly in the model. It might update retirementEligibilityDate.
}